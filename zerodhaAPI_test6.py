# IOC Order Margin Blocking Check 4
...
# Function 3: Execute All Orders with Optimized 1ms Delay
async def execute_all_orders():
    """
    Executes all orders with precise 1ms gaps using asyncio.create_task().
    """
    if not enctoken:
    ...
    async with aiohttp.ClientSession() as session:
        tasks = []
        start_time = asyncio.get_event_loop().time()  # High-precision timestamp

        for i, order in enumerate(orders):
            delay = i * 0.001  # 1ms delay per order
            tasks.append(asyncio.create_task(schedule_order(session, order, start_time + delay)))

        await asyncio.gather(*tasks)  # Fire all tasks
        

async def schedule_order(session, order, target_time):
    """
    Schedules an order to execute exactly at 'target_time'.
    """
    now = asyncio.get_event_loop().time()
    if now < target_time:
        await asyncio.sleep(target_time - now)  # Sleep until the precise moment

    await execute_order(session, order)  # Place the order
