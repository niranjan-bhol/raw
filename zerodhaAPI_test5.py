# IOC Order Margin Blocking Check 3
...
# Function 3: Execute All Orders with 1ms Delay
async def execute_all_orders():
    """
    Executes all orders sequentially with a 1ms delay between each order.
    """
    if not enctoken:
    ...
    async with aiohttp.ClientSession() as session:
        for order in orders:
            await execute_order(session, order)  # Place order
            await asyncio.sleep(0.001)  # Wait for 1 millisecond before placing the next order
